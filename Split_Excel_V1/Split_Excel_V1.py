...
#column = str(lines[1].rstrip()).lower() #第二行是column name
        ...
        if not os.path.exists(raw_path): #判断文件是否存在
            ...
            if split_x not in header:
                ...
                for line in split_list:
                    ...
                    writer = pd.ExcelWriter(f'{str(end_path)}\{str( line )} {str(file_date)}.xlsx',datetime_format="DD/MM/YYYY") #生成新文件名
                    excel_select.to_excel(writer, sheet_name='Sheet1', index=False) #29数据保存到新文件
